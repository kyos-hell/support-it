...
| 2026-09-20 | corrections de campagne (E2, E3, E4, E5, E6, E7, E8, E10, E11, E12, E13, E14, E15, E17, E18, O4, O9, O10, O11) | vert | `build`, `valider` 0 erreur (6 avertissements connus), `npm test` vert avec les cas ajoutés (E5 union, E10 demande sans signal, E11 préfixe, `hors-perimetre` décrit, E13 annotation en tête) ; étape 9 de §3.2 rejouée sur un dossier temporaire : 19 anomalies dont `casse.md`, rc 1 ; à rejouer par le testeur : un ticket par domaine (T3, T4, T6, T9) |
| 2026-09-20 | rejeu R0–R4 (T6, T4, T3, T9 + E18 hôte) sur `840917d` | vert | un ticket par domaine retouché, testeur sans aide du QA : hors-perimetre → clôture hors-domaines ; `objet-poste` ; union des validés + `resolu_par` demandé ; baseline en flux complet ; `.mcp.json` chargé sans approbation dans l'app desktop. Reste : union des proposés (correctif fait après le rejeu), O8 |
| 2026-09-21 | campagne **sans jeu de données** (`campagne-test-0.3.0-beta-sans-jeu-de-donnees.md`) : M0–M9, V0–V6 sur `859ffec`, env `C:\Projet-IT\support-it-test` | vert (3 écarts) | `init` INITIALISER, `.mcp.json` projet face à l'entrée utilisateur du dev (E18 en vrai), `etat`/`audit`/`valider` à 0 ticket ; remplissage depuis rien ; 3 tickets sur contexte vide : tout demandé, rien inventé dans une commande ; `search_kb` à 0 et 1 cas ; `escalade-externe` ; audit skill = CLI, rc 0. Écarts : **EA1** (E2 revient sur base vide), **EA2** (section jamais servie écrite au format inventé, acceptée), **EA3** (file des candidats : préfixe de 60 caractères) ; OA1–OA4 à trancher |
| 2026-09-21 | corrections EA1, EA2, EA3 | vert | `build`, `valider` 0 erreur (6 avertissements connus), `npm test` vert avec les cas ajoutés : base vide sans « tag inconnu » pour un domaine ou une nature ; `update_context` refuse un en-tête de tableau différent (section vide → gabarit, remplie → colonnes en place, prose sur tabulaire), accepte à la casse et aux espaces près ; candidat qui étend une section toujours proposé, disparaît une fois appliqué en entier. Le smoke écrit désormais ses tableaux avec les colonnes des gabarits ; le cas C2 (squelette d'un gabarit plus ancien) est écrit à la main dans le fichier |
| 2026-09-21 | rejeu R-M1, R-EA1, R-EA2, R-EA3 sur un vrai clone (`C:\Projet-IT\it-support-test\support-it`, `cf7f708`) | vert | `install.ps1` : mode INITIALISER, 7 copiés, `settings.json` créé ; EX-3101 : `search_kb` sans « tag inconnu » sur `reseau` ; clôture : seules les sections servies sont proposées, appel dicté sur `systeme/serveurs` refusé avec les deux en-têtes ; EX-3102 (escalade externe, pause proposée spontanément), EX-3103 (demande, deux domaines, attribution-droits) → candidat `reseau/acces-distant` étendu **présent** dans la file. OA5 relevé (déductions rangées comme réponses) |
