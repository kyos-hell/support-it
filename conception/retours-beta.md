...
## Après 0.3.0 (branche `release`, `228bf6a`)

| Date | Source | Constat | Couche | Gravité | Suite |
|---|---|---|---|---|---|
| 2026-09-23 | usage réel, hors projet | Session Claude Code ouverte dans aucun dossier, demande sur une VM : le skill `support` s'est chargé seul et a conduit un ticket. Deux causes. (1) `entree` copie le point d'entrée dans `~/.claude/skills/support/` (`cli.ts` l.280), portée utilisateur, donc visible dans toutes les sessions ; sa `description` (« dès qu'un technicien décrit un problème… ou un besoin ») le fait déclencher par le modèle sans `/support`. (2) Sur le poste de dev, `~/.claude.json` garde l'entrée MCP `support-it` de portée utilisateur (≤ 0.3.0-beta) : `enregistrer` ne la retire que s'il est relancé, il ne l'a pas été sur le dépôt de dev — hors projet, `load_skill` répond et les tickets vont dans `C:\Projet-IT\support-it\installation`. Sur un poste neuf 0.3.0, seul (1) joue : skill chargé, `load_skill` absent → le modèle improvise (contraire à « rien d'inventé »). E18 / décision 44 a passé le serveur en portée projet, pas le point d'entrée. | install, CLI, point d'entrée | **haut** | **Corrigé le 2026-09-23** (décision 50, `0.3.1`) : `entree` en portée projet + retrait de l'ancien, vérification dans `install.*` et `etat`, ligne « sans `load_skill`, arrête-toi », README ; `disable-model-invocation` non retenu (déclenchement automatique gardé, contenu au dossier). Doc Claude Code : skill personnel prioritaire sur projet ; skills de projet cherchés jusqu'à la racine git. À jouer : T-H6. Avant : À corriger, rien d'écrit : `entree` dépose le skill dans `<dossier de lancement>/.claude/skills/support/` (comme `hote` pour `settings.json`) et retire l'ancien `~/.claude/skills/support` s'il vient de ce produit ; doc : `outillage.md`, `fin-de-projet.md` §40, `validation.md`, `deploiement.md`. À trancher : `disable-model-invocation: true` (seul `/support` explicite déclenche). Poste de dev : relancer `enregistrer` pour retirer l'entrée utilisateur. |
| 2026-09-23 | T-H6 sur l'env de test (`d4d923c`, clone git) | Hors du dossier : ni skill ni serveur, `/support` inconnu — vert. Racine `support-it\` : triage servi, Bash et PowerShell absents des outils du modèle — vert. **Sous-dossier `support-it\produit\` : `/support` reconnu et `load_skill` servi (skill et `.mcp.json` trouvés en remontant jusqu'à la racine du dépôt), mais Bash et PowerShell présents** : le `.claude/settings.json` de la racine n'est pas appliqué depuis un sous-dossier. Un ticket y tourne sans les permissions refusées — le cas que la décision 50 voulait fermer, déplacé dans un sous-dossier. Pas une régression (même chose en 0.3.0). Le modèle a refusé `ipconfig` de lui-même : tenu par le prompt seul. L'encadré « From a subfolder » du README (0.3.1) est trompeur. Relevé aussi : ligne 1 du point d'entrée bien tenue pendant la connexion du serveur (quatre recherches, pas d'abandon, pas d'improvisation). | hôte, install, README | **haut** | Tranché le 2026-09-23 : `0.3.1` livrée avec la limite dite (README, messages de fin, `fin-de-projet.md` §14) ; la garde en `0.3.2`. Pistes : refus côté serveur si le dossier de la session n'est pas le dossier de lancement (à vérifier : le `cwd` du processus MCP est-il celui de la session ?) ; README et messages : « jamais depuis un sous-dossier » ; rejouer la même question depuis un sous-dossier hors git (archive). |
