...
- **Toute vérification passe par le technicien ; toute exécution aussi.**
  Je produis un plan, je ne modifie rien. Jamais de mot de passe dans la
  conversation : le plan dit *comment* il est transmis, pas *lequel*.
...
- Demande absente de ce fichier : le dire, instruire au mieux avec le
  ...
  ajouter. Boîte, partage, poste : système ou poste de travail, le dire.

## Demande — creation-compte

**Prérequis.** La demande vient d'une source légitime (`identite/cycle-de-vie` :
qui peut demander une arrivée) ; l'annuaire d'autorité pour ce type de
compte est identifié (`identite/annuaires`).
...
**Gabarit de plan d'action.** Compte (nom, OU ou conteneur, attributs),
groupes dans l'ordre, date d'expiration si temporaire, remise du premier
mot de passe (canal, jamais la valeur), enregistrement MFA, délai de
propagation à annoncer au demandeur, impact (aucun sur l'existant), retour
arrière (désactivation, pas suppression), et la mise à jour de
`identite/cycle-de-vie` seulement si le processus a été précisé en route.

## Demande — depart-collaborateur

**Prérequis.** Date de départ et source légitime confirmées
(`identite/cycle-de-vie`) ; le compte est localisé dans son annuaire
d'autorité.
...
des données (durée, propriétaire), suppression différée à l'échéance,
impact (services portés par le compte, à traiter avant), retour arrière
(réactivation jusqu'à la suppression), mise à jour de
`identite/comptes-service` seulement si un compte de service est découvert.
...
approbation nommée, date de fin si temporaire, délai de prise d'effet
(nouvelle session), impact (aucun sur les autres membres), retour arrière
(retrait du groupe), et la mise à jour de `identite/groupes-droits` si un
groupe a été créé pour l'occasion.
